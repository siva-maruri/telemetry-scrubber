...
class Tokenizer:
    ...
    @classmethod
    def from_env(
        cls, var: str = "SCRUBBER_TOKEN_KEY", version_var: str = "SCRUBBER_TOKEN_KEY_VERSION"
    ) -> Tokenizer:
    ...
    @classmethod
    def from_key_vault(cls, vault_url: str, secret_name: str, version: str | None = None) -> Tokenizer:
        ...
        if not secret.value or not secret.properties.version:
            raise RuntimeError(f"secret {secret_name} in {vault_url} has no value")
